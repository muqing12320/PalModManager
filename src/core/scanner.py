...
class ModScanner:
    ...
    def scan_pak_mods(self) -> List[ModInfo]:
        ...
        # Collect all .pak files including in subdirectories (e.g. ~mods/MapFastTravelAnywhere/)
        def _collect_paks(root: Path) -> List[Path]:
            result = []
            for item in root.iterdir():
                if item.is_file() and item.suffix.lower() == '.pak':
                    result.append(item)
                elif item.is_dir():
                    result.extend(_collect_paks(item))
            return result
        
        for item in _collect_paks(paks_dir):
    ...
    def scan_logicmods_paks(self) -> List[ModInfo]:
        ...
        # Helper: collect all .pak files (including in subdirs)
        def _collect_pak_files(root: Path) -> List[Path]:
            result = []
            for item in root.iterdir():
                if item.is_file() and item.suffix.lower() == '.pak':
                    result.append(item)
                elif item.is_dir():
                    result.extend(_collect_pak_files(item))
            return result
        
        for pak_file in _collect_pak_files(logicmods_dir):
            stem = pak_file.stem.lower()
            seen_stems.add(stem)
            mod_info = self._scan_pak_file(pak_file)
            if mod_info:
